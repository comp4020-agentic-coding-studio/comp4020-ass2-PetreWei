...
- **Task.** Get rules that actually bind the agent, wherever they come from.
...
- **Result.** The diff showed most of the generated rules were weaker versions of ones the inherited file already had. `git status` caught the push, not the subagent's report. The artefact is replaced each deliverable; the harness accumulates.

### 2.3 What No Check Could Catch

Reading the twelve weeks side by side found what eight green checks could not.

- **Situation.** Every week was written and the suite green, which says nothing about whether twelve weeks argue one thing.
