...
- **Write each moment in STAR form, weighted.** Situation 20% — enough context to place the example, not every detail; Task 10% — what you took responsibility for; Action 60% — what you personally did, and the skill it shows; Result 10% — the outcome, quantified where it can be, and what you took from it. The weighting is the rule: an even split just recounts events, while 60% on your own actions is what makes a capability visible. Name the method, not only the outcome it reached.
