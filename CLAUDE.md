...
- **List the plan's premises separately from its steps.** Mark each as the brief's, the client's, or mine. Reviewing only the steps cannot find an error the plan and the reviewer share, and that is the one that survives to become structural.
...
- **Say where a rule or a framing came from.** The brief, the spec, tutor feedback, or my own judgement. An organising idea that is not traceable to the brief or `src/course-config.ts` is mine, and it says so when proposed. An invented convention that reads as an inherited one cannot be argued with, because nobody knows it is open to argument.
- **Retrying is a decision about state you cannot observe.** A timeout does not tell you whether the server already committed, so the course does not describe it as a purchase or an exchange. `spec/week-distinctness.test.ts` holds the retired word list. The engineering limits those words used to carry are still stated, as facts about mechanism: backoff delays requests that were about to succeed, an open breaker refuses calls that would have worked, a deadline discards work that was nearly finished.
...
- **A sentence that counts pages or points at another page is a claim the build does not check.** "Five of the twelve come with a full deck" was falsified by adding decks, in two places at once. Prefer prose that stays true when a file is added; where a count or a cross-reference earns its place, a check owns it.
...
- **`/zh/` is a pilot covering the home page and the nav.** There is no i18n `fallback`, so an untranslated `/zh/*` URL 404s; every Chinese card pointing at an English page carries `（该页面暂为英文）`; the tag gloss is display-only, and `src/course-config.ts` stays the source of truth. Write the Chinese from the course's ideas rather than translating the English, and have a reader of Chinese judge it, because the build cannot.
...
- **Spend verification where the pages differ.** A change touching twelve similar pages is driven in the browser on the one or two that are genuinely distinct, with `pnpm check` covering the rest. Read the console once, extract the value needed instead of capturing the whole page, and do not re-observe what has not changed.
...
- **Write the assertion so it can only pass for the right reason.** Assert that the thing is used, because a forbid-only check is satisfied by an empty page. Give a compound promise one assertion per claim, or it passes on the easy half. Name the offending value in the message, so a failure says what to go and look at. A one-off verification script reports its denominator for the same reason: one built on a field that did not exist reported clean having swept 7 pages of 42.
